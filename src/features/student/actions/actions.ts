...
export default function extraReducers(builder: ActionReducerMapBuilder<State>) {
  builder
    ...
    .addCase(addFollowers, (student, action) => {
      ...
      const followers = action.payload.followers.filter(
        (follower) => !student.followers?.includes(follower)
      )
    ...
    .addCase(removeFollowers, (student, action) => {
      ...
      student.followers = student.followers.filter(
        (follower) => !action.payload.followers.includes(follower)
      )
    })
